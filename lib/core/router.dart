...
class PageRouter<RouteKey> extends GetxController
  ...
  int get bottomNavigationIdx => _bottomNavigationIdx.value;
  ...
  @override
  void goToTab(RouteKey target) {
    assert(routes.keys.contains(target), "target 必须定义在 routes");
    assert(routes[target]!.type == RouteType.bottomNavigation,
  ...
  void go(RouteKey target) {
    assert(routes.keys.contains(target), "target 必须定义在 routes");
    assert(routes[target]!.type == RouteType.page, "goToTab 必须跳转到 page 类型的路由");
